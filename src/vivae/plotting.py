...
from .geometry import DecoderIndicatome, EncoderIndicatome
...
def plot_embedding(
    ...
    if labels is not None:
        ...
        if unassigned is not None:
            ...
            for upop in np.array(unassigned):
                ...
                if len(idcs) > 0:
                    ...
                    la = np.delete(la, idcs)
        idx_pop = 0
        for pop in np.unique(la):
            ## Plot assigned points
            idcs = np.where(la == pop)[0]
            ax.scatter(
                embedding[idcs, 0],
                embedding[idcs, 1],
                label=pop,
        ...
        if not (draw_mst and fsom_view == "markers" and fsom_markers is not None):
            la = plt.legend(bbox_to_anchor=bbox_to_anchor, loc="upper left", markerscale=30)

            for lh in la.legend_handles:
                lh.set_alpha(1.0)
                lh._sizes = [50]
    ...
    if draw_mst:
        ...
        cluster_empty = cluster_sizes == 0.0
        ...
        if fsom_show_nodes:
            ...
            if fsom_view == "labels" and labels is not None:
                ...
                color_dict = dict(zip(pops, palette, strict=False))
                if unassigned is not None:
                    for u in unassigned:
                        color_dict[u] = "#bfbfbf"
                ...
                if len(markers) == 1:
                    ...
                    indices_markers = (np.asarray(fsom.get_cell_data().var_names)[:, None] == ref_markers).argmax(
                        axis=0
                    )
                    lim = (
                        mfis[:, indices_markers].min(),
                        mfis[:, indices_markers].max(),
                    )
                    ...
                    n.set_clim(lim)
                    n.set_edgecolor("black")
                    n.set_linewidth(fsom_edge_scale / 4.0)
                    n.set_zorder(3)
                    ax.add_collection(n)
                    ...
                    fig, ax = fs.pl._plot_helper_functions.plot_star_legend(
                        ...
                        coords=(max_x, max_y),
                        cmap=cmap,
                        max_star_height=np.max(node_sizes) * 3.0,
                        star_height=1.0,
                    )

                    data = fsom.get_cluster_data()[:, markers].X
                    heights = fs.pl._plot_helper_functions.scale_star_heights(data, node_sizes)
                    s = mc.PatchCollection(
                        fs.pl._plot_helper_functions.add_stars(layout, heights),
                        cmap=cmap,
                    )
                    s.set_array(range(data.shape[1]))
                    s.set_edgecolor("black")
                    s.set_linewidth(fsom_edge_scale / 4.0)
                    s.set_zorder(3)
                    ax.add_collection(s)
                    plt.axis("off")
                ...
                ax = fs.pl._plot_helper_functions.add_text(
                    ax,
                    layout,
                    numbers,
                    text_size=fsom_text_size,
                    text_color="black",
                    ha=["center"],
                )
